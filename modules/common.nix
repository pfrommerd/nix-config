{ config, pkgs, lib, ...}: {
  ...
  config = let cfg = config.distro.common;
    ...
    programs.nix-ld.libraries = [
    	pkgs.libGL pkgs.glib pkgs.libcxx pkgs.libgcc
    ...
    security.pki.certificateFiles = [ age.secrets.root-crt.path ];
    services.caddy = {
      enable = cfg.proxy != {};
      globalConfig = ''
        pki {
      ...
      virtualHosts = lib.mapAttrs (name: target: {
        extraConfig = if target == "helloworld" then ''
    ...
    environment.etc."keys/root.key" = lib.mkIf (cfg.proxy != {}) {
      source = ./keys/root.key;
      mode = "0440"; user = "caddy"; group = "caddy";
    };
    environment.etc."keys/root.crt" = lib.mkIf (cfg.proxy != {}) {
      source = ./keys/root.crt;
      mode = "0440"; user = "caddy"; group = "caddy";
    };
  };
}
